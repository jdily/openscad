...
*/
			...
			fwrite(CompactColor, sizeof(unsigned char), ANGLE * CAMNUM * sizeof(uint64_t), fpt4);
			sprintf(filename, "%s.ccd", fname);
			if( (fpt = fopen(filename, "wb")) == NULL )	{	printf("Write %s error!!\n", filename);	return;	}
			// fwrite(CompactColor, sizeof(unsigned char), ANGLE * CAMNUM * sizeof(unsigned __int64), fpt);
			fwrite(CompactColor, sizeof(unsigned char), ANGLE * CAMNUM * sizeof(uint64_t), fpt);
			fclose(fpt);
		}
...
// *************************************************************************************************
	...
	case 'x':
		...
		fread(CompactColor, ANGLE * CAMNUM, sizeof(uint64_t), fpt);
		...
		while( fscanf(fpt1, "%s", destfn) != EOF )
		{
			...
			fread(dest_CompactColor, ANGLE * CAMNUM,  sizeof(uint64_t), fpt);
